...
import { Button, Drawer, Form, FormItem, Input, Modal, Select, Table, Tag, message } from 'antdv-next'
...
async function load() {
  ...
  } catch (error) { console.error(error); message.error('加载同步身份源失败') } finally { loading.value = false }
}
  ...
  <div class="p-6 min-h-[calc(100vh-64px)] space-y-6">
    <section class="rounded-xl border bg-card p-6 shadow-sm">
      <div class="flex flex-wrap items-start justify-between gap-4">
        <div class="flex gap-3"><Building2 class="mt-1 text-primary size-6" /><div><h1 class="text-2xl font-bold text-foreground">同步身份源</h1><p class="mt-1 text-sm text-muted-foreground">通过飞书通讯录同步组织机构和 pool_user；不用于登录授权。</p></div></div>
    ...
    <section class="rounded-xl border bg-card p-5 shadow-sm">
      <div class="mb-4 flex flex-wrap gap-3"><div class="w-full sm:w-60"><Input v-model:value="search" placeholder="搜索身份源名称" @press-enter="load" /></div><div class="w-full sm:w-32"><Select v-model:value="status" allow-clear class="w-full" placeholder="状态" :options="statusOptions" /></div><Button @click="load"><RefreshCw class="size-4 mr-2" /> 查询</Button></div>
    ...
    <Drawer v-model:open="drawerOpen" :title="editing ? '编辑飞书身份源' : '添加飞书身份源'" size="large" destroy-on-hidden>
      <Form layout="vertical" :model="form"><FormItem label="身份源名称" required><Input v-model:value="form.name" placeholder="例如：总部飞书通讯录" /></FormItem><FormItem label="飞书 App ID" required><Input v-model:value="form.appId" placeholder="cli_xxx" /></FormItem><FormItem :label="editing ? 'App Secret（留空则不修改）' : 'App Secret'" required><Input v-model:value="form.appSecret" type="password" /></FormItem><FormItem :label="editing ? 'Verification Token（留空则不修改）' : 'Verification Token'" required><Input v-model:value="form.verificationToken" type="password" /></FormItem><FormItem :label="editing ? 'Encrypt Key（留空则不修改）' : 'Encrypt Key'" required><Input v-model:value="form.encryptKey" type="password" /></FormItem><FormItem label="状态"><Select v-model:value="form.status" :options="statusOptions" /></FormItem></Form>
