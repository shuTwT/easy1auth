...
public interface ApplicationRepository extends JRepository<OAuthApplicationEntity, UUID> {
    ...
    default boolean nameExists(String name, UUID excluding) {
        return sql()
                .createQuery(APP)
                .where(APP.name().eq(name))
                .whereIf(excluding != null, () -> APP.id().ne(excluding))
                .select(APP.id())
                .exists();
    }
    ...
    default List<OAuthApplicationEntity> listPage(
        ...
        return sql()
                ...
                .orderBy(APP.createdAt().desc())
                .select(APP)
                .limit(pageSize, (long) (page - 1) * pageSize)
                .execute();
    }

    default long count(String name, String type, String status) {
        return sql()
                .createQuery(APP)
                .whereIf(name != null && !name.isBlank(), () -> APP.name().ilike(name, LikeMode.ANYWHERE))
                .whereIf(type != null && !type.isBlank(), () -> APP.type().eq(type))
    ...
    default Optional<OAuthApplicationEntity> findActiveByClientId(String clientId) {
        return sql()
                .createQuery(APP)
                .where(APP.clientId().eq(clientId), APP.status().eq("active"))
                .select(APP)
                .fetchOptional();
    }
    ...
    default Optional<OAuthApplicationEntity> findActive(UUID tenant, UUID id) {
        return sql()
                .createQuery(APP)
                .where(APP.id().eq(id), APP.tenantId().eq(tenant), APP.status().eq("active"))
                .select(APP)
                .fetchOptional();
    }
    ...
    default int update(
        ...
        var u =
                sql()
                        ...
                        .set(APP.type(), type)
                        .set(APP.redirectUris(), redirects)
                        .set(APP.postLogoutRedirectUris(), logout)
                        .set(APP.allowedGrantTypes(), grants)
                        .set(APP.scopes(), scopes)
                        .set(APP.requirePkce(), pkce)
                        .set(APP.requireConsent(), consent)
                        .set(APP.accessTokenLifetime(), access)
                        .set(APP.refreshTokenLifetime(), refresh)
                        .set(APP.updatedAt(), Instant.now())
                        .where(APP.id().eq(id), APP.tenantId().eq(tenant));
        if (secretHash != null) {
            u.set(APP.clientSecretHash(), secretHash);
        }
        return u.execute();
    }
