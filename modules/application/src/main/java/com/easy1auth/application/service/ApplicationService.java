 ...
 *
 * <p>负责应用的创建、分页查询、详情、更新、删除、启停、密钥重新生成与统计， 并对类型、授权类型、重定向 URI、令牌有效期等做归一化与合规校验；创建与更新
 * 会校验租户应用配额与应用名称的唯一性。
 */
@Service
public class ApplicationService {
  ...
  public ApplicationService(
      ApplicationRepository repository, TenantService tenants, PasswordEncoder passwords) {
  ...
  public ApplicationView create(ApplicationInput input) {
    ...
    if (repository.nameExists(normalized.name(), null)) {
    ...
    var entity =
        OAuthApplicationEntityDraft.$.produce(
            d ->
                d.setId(id)
                    .setTenantId(tenant)
                    ...
                    .setScopes(normalized.scopes())
                    .setRequirePkce(normalized.requirePkce())
  ...
  public PageData<ApplicationView> list(
      int page, int pageSize, String name, String type, String status) {
    int p = Math.max(1, page), size = Math.min(100, Math.max(1, pageSize));
    long total = repository.count(name, type, status);
    return PageData.of(
        repository.listPage(p, size, name, type, status).stream().map(e -> view(e, null)).toList(),
        p,
        size,
        total);
  }
  ...
  public OAuthApplicationEntity findActiveByClientId(String clientId) {
    return repository.findActiveByClientId(clientId).orElse(null);
  }

  /** 按 ID 查询指定租户下 active 状态的应用（供授权服务器使用）。 */
  @Transactional(readOnly = true)
  public OAuthApplicationEntity findActive(UUID tenant, UUID id) {
    return repository.findActive(tenant, id).orElse(null);
  }
  ...
  public ApplicationView update(UUID id, ApplicationInput input) {
    ...
    if (!old.name().equals(normalized.name()) && repository.nameExists(normalized.name(), id)) {
      throw new DomainException(ErrorCodeConstants.APPLICATION_NAME_EXISTS);
    }
    String oneTimeSecret = null;
    String secretHash = null;
    boolean updateSecret = false;
    if (isPublic(old.type()) && !isPublic(normalized.type())) {
      oneTimeSecret = secret();
      secretHash = passwords.encode(oneTimeSecret);
      updateSecret = true;
    ...
    repository.update(
        ...
        normalized.allowedGrantTypes(),
        normalized.scopes(),
        normalized.requirePkce(),
        normalized.requireConsent(),
        normalized.accessTokenLifetime(),
        normalized.refreshTokenLifetime(),
        secretHash,
        updateSecret);
  ...
  public ApplicationView status(UUID id, String status) {
    ...
    entity(id);
    repository.updateStatus(tenant, id, status);
    return get(id);
  }
  ...
  public SecretView regenerateSecret(UUID id) {
    ...
    repository.updateSecret(tenant, id, passwords.encode(secret));
  ...
  public ApplicationStats stats() {
    var rows = repository.statuses();
    return new ApplicationStats(
        rows.size(),
        rows.stream().filter("active"::equals).count(),
        rows.stream().filter("disabled"::equals).count());
  }
  ...
  private static ApplicationInput normalize(ApplicationInput in, boolean creating) {
    ...
    List<String> grants =
        in.allowedGrantTypes() == null ? defaultGrants(type) : copy(in.allowedGrantTypes());
    ...
    List<String> scopes =
        in.scopes() == null ? List.of("openid", "profile", "email", "phone") : copy(in.scopes());
    int access = in.accessTokenLifetime() == null ? 900 : in.accessTokenLifetime(),
        refresh = in.refreshTokenLifetime() == null ? 2592000 : in.refreshTokenLifetime();
    ...
    boolean pkce =
        in.requirePkce() != null ? in.requirePkce() : grants.contains("authorization_code");
    ...
    return new ApplicationInput(
        ...
        scopes,
        pkce,
        in.requireConsent() == null || in.requireConsent(),
        access,
        refresh);
  }
  ...
  private static List<String> defaultGrants(String type) {
    return "machine".equals(type)
        ? List.of("client_credentials")
        : List.of("authorization_code", "refresh_token");
  }

  /** 用传入输入覆盖旧值（未传字段沿用旧值）后进行归一化校验。 */
  private static ApplicationInput normalizeForUpdate(
      OAuthApplicationEntity old, ApplicationInput in) {
    if (in == null) {
      return from(old);
    }
    return normalize(
        new ApplicationInput(
            in.name() == null ? old.name() : in.name(),
            in.logo() == null ? old.logo() : in.logo(),
            in.description() == null ? old.description() : in.description(),
            in.type() == null ? old.type() : in.type(),
            in.redirectUris() == null ? old.redirectUris() : in.redirectUris(),
            in.postLogoutRedirectUris() == null
                ? old.postLogoutRedirectUris()
                : in.postLogoutRedirectUris(),
            in.allowedGrantTypes() == null ? old.allowedGrantTypes() : in.allowedGrantTypes(),
            in.scopes() == null ? old.scopes() : in.scopes(),
            in.requirePkce() == null ? old.requirePkce() : in.requirePkce(),
            in.requireConsent() == null ? old.requireConsent() : in.requireConsent(),
            in.accessTokenLifetime() == null ? old.accessTokenLifetime() : in.accessTokenLifetime(),
            in.refreshTokenLifetime() == null
                ? old.refreshTokenLifetime()
                : in.refreshTokenLifetime()),
        false);
  }
