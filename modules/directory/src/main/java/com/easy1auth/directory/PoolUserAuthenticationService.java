...
public class PoolUserAuthenticationService {
    ...
    private final JSqlClient sql;private final PasswordEncoder passwords;private final LoginProtectionService protection;private final SecurityPolicyService security;
    public PoolUserAuthenticationService(JSqlClient sql,PasswordEncoder passwords,LoginProtectionService protection,SecurityPolicyService security){this.sql=sql;this.passwords=passwords;this.protection=protection;this.security=security;}
    @Transactional
    public PoolPrincipal authenticate(UUID tenant,String login,String password){
        if(login==null||password==null)return null;String key=login.strip().toLowerCase();var policy=security.policy(tenant);protection.assertAllowed("pool_user",key,tenant);
        ...
        if(user==null||user.passwordHash()==null||!passwords.matches(password,user.passwordHash())){protection.failed("pool_user",key,tenant,policy.loginAttemptLimit(),policy.lockoutSeconds());return null;}protection.succeeded("pool_user",key,tenant);
