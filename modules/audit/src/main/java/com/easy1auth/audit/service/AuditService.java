...
public class AuditService {
    ...
    public UUID record(Event input) {
        ...
        var e = AuditEventEntityDraft.$.produce(
                d -> d.setId(id)
                        .setTenantId(input.tenantId())
                        .setActorType(clean(input.actorType(), 24, "system"))
                        .setActorId(input.actorId())
                        .setActorName(trim(input.actorName(), 320))
                        .setEventType(clean(input.eventType(), 100, "security"))
                        .setAction(clean(input.action(), 100, "unknown"))
                        .setResourceType(clean(input.resourceType(), 100, "unknown"))
                        .setResourceId(trim(input.resourceId(), 200))
                        .setTraceId(trim(input.traceId(), 100))
                        .setMethod(trim(input.method(), 16))
                        .setIpAddress(trim(input.ipAddress(), 64))
                        .setUserAgent(trim(input.userAgent(), 512))
                        .setOutcome("failure".equals(input.outcome()) ? "failure" : "success")
                        .setErrorCode(trim(input.errorCode(), 100))
                        .setDetails(redact(input.details()))
    ...
    public PageData<AuditEventEntity> list(int page, int size, Query q) {
        ...
        var query = repository.sql()
                .createQuery(EVENT)
                .where(EVENT.tenantId().eq(tenant))
                .whereIf(q != null && q.actorName() != null && !q.actorName().isBlank(), () -> EVENT.actorName().ilike(q.actorName(), LikeMode.ANYWHERE))
                .whereIf(q != null && q.eventType() != null && !q.eventType().isBlank(), () -> EVENT.eventType().eq(q.eventType()))
                .whereIf(q != null && q.action() != null && !q.action().isBlank(), () -> EVENT.action().eq(q.action()))
                .whereIf(q != null && q.outcome() != null && !q.outcome().isBlank(), () -> EVENT.outcome().eq(q.outcome()))
                .whereIf(q != null && q.start() != null, () -> EVENT.createdAt().ge(q.start()))
                .whereIf(q != null && q.end() != null, () -> EVENT.createdAt().le(q.end()))
