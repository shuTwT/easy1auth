 ...
 */
public interface TenantPackageRepository extends JRepository<TenantPackageEntity, Long> {
    /** tenant_package 表静态描述符 */
    TenantPackageEntityTable PACKAGE = TenantPackageEntityTable.$;
    /** tenant_package_permission 表静态描述符 */
    TenantPackagePermissionEntityTable PACKAGE_PERMISSION = TenantPackagePermissionEntityTable.$;
    /** tenant 表静态描述符 */
    TenantEntityTable TENANT = TenantEntityTable.$;

    /** 查询全部套餐，按创建时间与 ID 倒序。 */
    default List<TenantPackageEntity> list() {
    ...
    /** 按 ID 查询套餐。 */
    default Optional<TenantPackageEntity> find(long packageId) {
        return sql().createQuery(PACKAGE)
    ...
    default Optional<TenantPackageEntity> lock(long packageId) {
        return sql().createQuery(PACKAGE)
                .where(PACKAGE.id().eq(packageId))
                .select(PACKAGE)
                .forUpdate()
    ...
    default Optional<TenantPackageEntity> lockActiveDefault() {
        return sql().createQuery(PACKAGE)
    ...
    /** 按编码查询套餐。 */
    default Optional<TenantPackageEntity> findByCode(String code) {
        return sql().createQuery(PACKAGE)
    ...
    default Optional<TenantPackageEntity> findActive(long packageId) {
        return sql().createQuery(PACKAGE)
    ...
    default boolean codeExists(String code, Long excludingId) {
        return sql().createQuery(PACKAGE)
                .where(PACKAGE.code().eq(code))
    ...
    default boolean nameExists(String name, Long excludingId) {
        return sql().createQuery(PACKAGE)
    ...
    default boolean isReferencedByOrdinaryTenant(long packageId) {
    ...
    /** 新建套餐（status=active）。 */
    default void create(TenantPackageMutation mutation) {
    ...
    /** 更新套餐的编码、名称、默认标记与配额。 */
    default void update(long packageId, TenantPackageMutation mutation) {
    ...
    default void updateStatus(long packageId, String status) {
        sql().createUpdate(PACKAGE)
    ...
    default void delete(long packageId) {
    ...
    /** 查询套餐绑定的权限编码，按编码升序返回。 */
    default List<String> permissionCodes(long packageId) {
    ...
    default void replacePermissions(long packageId, Collection<String> permissionCodes) {
