...
public class AdminIdentityService implements ActiveAdminAccountLocker {
    ...
    public AuthenticatedAdmin authenticate(String login, String password, Duration refreshTtl, String userAgent, String ip) {
        ...
        String key=AdminIdentityNormalizer.normalizeLogin(login);protection.assertAllowed("admin",key,null);
        var found=repository.findCredential(key);if(found.isEmpty()){protection.failed("admin",key,null,5,1800);throw invalidCredentials();}var row=found.get();
    ...
    @Transactional public void resetPassword(UUID actor,UUID id,String password) { assertNotActor(actor,id,"SELF_PASSWORD_RESET_DENIED","不能重置自己的密码"); validatePassword(password); repository.resetPassword(id,passwords.encode(password)); repository.revokeAll(id); }
    ...
    @Transactional public AdminAccount resetMfa(UUID actor,UUID id) { assertNotActor(actor,id,"SELF_MFA_RESET_DENIED","不能重置自己的 MFA"); var account=repository.resetMfa(id); repository.revokeAll(id); return account; }
    @Transactional public AdminAccount disableMfa(UUID id) { var account=repository.resetMfa(id); repository.revokeAll(id); return account; }
