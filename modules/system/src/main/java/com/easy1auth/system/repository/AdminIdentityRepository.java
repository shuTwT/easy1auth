 ...
 */
public interface AdminIdentityRepository extends JRepository<AdminAccountEntity, UUID> {
    /**
     ...
     */
    AdminCredentialEntityTable CREDENTIAL = AdminCredentialEntityTable.$;
    /**
     * admin_refresh_session 表静态描述符
     */
    AdminRefreshSessionEntityTable SESSION = AdminRefreshSessionEntityTable.$;

    /**
     * 按登录标识（用户名或邮箱，忽略大小写）查询账号及其密码哈希，用于登录校验。
     */
    default Optional<CredentialRow> findCredential(String login) {
     ...
     */
    default Optional<AdminAccount> findActive(UUID id) {
     ...
     */
    default Optional<AdminAccount> findActiveByEmail(String email) {
     ...
     */
    default Optional<AdminAccount> lockActive(UUID id) {
     ...
     */
    default boolean exists(String username, String email) {
     ...
     */
    default AdminAccount create(String username, String email, String hash) {
     ...
     * 记录账号最近登录时间。
     */
    default void recordLogin(UUID id) {
     ...
     */
    default void createSession(UUID id, UUID account, String hash, long version, Instant expires, String agent, String ip) {
        Instant now = Instant.now();
     ...
     */
    default Optional<SessionRow> lockSession(String hash) {
     ...
     */
    default void rotate(UUID oldId, UUID replacement) {
     ...
     * 按令牌哈希吊销刷新会话（用于退出登录）。
     */
    default void revoke(String hash) {
     ...
     * 吊销某账号的全部刷新会话。
     */
    default void revokeAll(UUID account) {
     ...
     */
    default void invalidateAccountSessions(UUID account) {
     ...
     */
    default AdminAccount updateProfile(UUID id, String username, String email, String phone) {
     ...
     */
    default boolean emailExistsForOtherAccount(UUID id, String email) {
     ...
     */
    default AdminAccount updateOwnProfile(UUID id, String username, String phone) {
     ...
     */
    default AdminAccount changeOwnEmail(UUID id, String email) {
        if (sql().createUpdate(ACCOUNT)
     ...
     */
    default AdminAccount updateStatus(UUID id, String status) {
     ...
     */
    default void resetPassword(UUID id, String hash) {
     ...
     * 关闭账号 MFA 并递增安全版本号。
     */
    default AdminAccount resetMfa(UUID id) {
     ...
     * 开启账号 MFA 并递增安全版本号。
     */
    default AdminAccount enableMfa(UUID id, String type) {
     ...
     * 递增账号安全版本号（使旧令牌/会话失效）。
     */
    default void incrementVersion(UUID id) {
     ...
     * 按 ID 查询账号（不限状态）。
     */
    default Optional<AdminAccount> findAccount(UUID id) {
     ...
     * 将账号实体转换为领域模型。
     */
    static AdminAccount toDomain(AdminAccountEntity a) {
