...
import com.easy1auth.admin.security.TenantContextFilter; import com.easy1auth.federation.FederationService; import com.easy1auth.foundation.web.ApiResponse; import com.easy1auth.tenant.TenantContext;
...
@RestController @RequestMapping("/api/social-identity-providers") public class FederationController {
 ...
 @GetMapping ApiResponse<?> list(HttpServletRequest r,@RequestParam(defaultValue="1")int page,@RequestParam(defaultValue="10")int pageSize,@RequestParam(required=false)String search,@RequestParam(required=false)String status){return ApiResponse.ok(service.list(c(r).tenantId(),page,pageSize,search,status));}
