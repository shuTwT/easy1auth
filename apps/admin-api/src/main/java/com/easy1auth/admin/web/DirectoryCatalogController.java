...
public class DirectoryCatalogController {
    ...
    ApiResponse<?> groupTree(HttpServletRequest r) {
        return ApiResponse.ok(catalog.groupTree(c(r).tenantId()));
    }

    @TenantManagementPermission(value = ManagementPermissionCode.GROUP_STATS)
    @GetMapping("/api/groups/stats")
    ApiResponse<?> groupStats(HttpServletRequest r) {
        return ApiResponse.ok(catalog.groupStats(c(r).tenantId()));
    }

    @TenantManagementPermission(value = ManagementPermissionCode.GROUP_READ)
    @GetMapping("/api/groups/{id}")
    ApiResponse<?> group(HttpServletRequest r, @PathVariable UUID id) {
        return ApiResponse.ok(catalog.group(c(r).tenantId(), id));
    }

    @TenantManagementPermission(value = ManagementPermissionCode.GROUP_CREATE)
    @PostMapping("/api/groups")
    ApiResponse<?> createGroup(HttpServletRequest r, @RequestBody DirectoryCatalogService.GroupInput in) {
        return ApiResponse.ok(catalog.createGroup(c(r).tenantId(), in), "用户组创建成功");
    }

    @TenantManagementPermission(value = ManagementPermissionCode.GROUP_UPDATE)
    @PutMapping("/api/groups/{id}")
    ApiResponse<?> updateGroup(HttpServletRequest r, @PathVariable UUID id, @RequestBody DirectoryCatalogService.GroupInput in) {
        return ApiResponse.ok(catalog.updateGroup(c(r).tenantId(), id, in), "用户组更新成功");
    }

    @TenantManagementPermission(value = ManagementPermissionCode.GROUP_DELETE)
    @DeleteMapping("/api/groups/{id}")
    ApiResponse<Void> deleteGroup(HttpServletRequest r, @PathVariable UUID id) {
        catalog.deleteGroup(c(r).tenantId(), id);
    ...
    ApiResponse<?> groupMembers(HttpServletRequest r, @PathVariable UUID id) {
        return ApiResponse.ok(catalog.groupMembers(c(r).tenantId(), id));
    }

    @TenantManagementPermission(value = ManagementPermissionCode.GROUP_MEMBERS_ADD)
    @PostMapping("/api/groups/{id}/members")
    ApiResponse<Void> addMembers(HttpServletRequest r, @PathVariable UUID id, @RequestBody UserIds in) {
        catalog.addMembers(c(r).tenantId(), id, in.userIds());
    ...
    ApiResponse<Void> removeMembers(HttpServletRequest r, @PathVariable UUID id, @RequestBody UserIds in) {
        catalog.removeMembers(c(r).tenantId(), id, in.userIds());
    ...
    ApiResponse<Void> addAdmins(HttpServletRequest r, @PathVariable UUID id, @RequestBody UserIds in) {
        catalog.addAdmins(c(r).tenantId(), id, in.userIds());
    ...
    ApiResponse<Void> removeAdmins(HttpServletRequest r, @PathVariable UUID id, @RequestBody UserIds in) {
        catalog.removeAdmins(c(r).tenantId(), id, in.userIds());
    ...
    ApiResponse<?> positionStats(HttpServletRequest r) {
        return ApiResponse.ok(catalog.positionStats(c(r).tenantId()));
    }

    @TenantManagementPermission(value = ManagementPermissionCode.POSITION_READ)
    @GetMapping("/api/positions/{id}")
    ApiResponse<?> position(HttpServletRequest r, @PathVariable UUID id) {
        return ApiResponse.ok(catalog.position(c(r).tenantId(), id));
    }

    @TenantManagementPermission(value = ManagementPermissionCode.POSITION_CREATE)
    @PostMapping("/api/positions")
    ApiResponse<?> createPosition(HttpServletRequest r, @RequestBody DirectoryCatalogService.PositionInput in) {
        return ApiResponse.ok(catalog.createPosition(c(r).tenantId(), in), "岗位创建成功");
    }

    @TenantManagementPermission(value = ManagementPermissionCode.POSITION_UPDATE)
    @PutMapping("/api/positions/{id}")
    ApiResponse<?> updatePosition(HttpServletRequest r, @PathVariable UUID id, @RequestBody DirectoryCatalogService.PositionInput in) {
        return ApiResponse.ok(catalog.updatePosition(c(r).tenantId(), id, in), "岗位更新成功");
    }

    @TenantManagementPermission(value = ManagementPermissionCode.POSITION_DELETE)
    @DeleteMapping("/api/positions/{id}")
    ApiResponse<Void> deletePosition(HttpServletRequest r, @PathVariable UUID id) {
        catalog.deletePosition(c(r).tenantId(), id);
