...
public class PoolUserImportController {
    ...
    ResponseEntity<byte[]> template() throws IOException {
        try (var workbook = new XSSFWorkbook(); var output = new ByteArrayOutputStream()) {
            ...
            for (int i = 0; i < columns.length; i++) { header.createCell(i).setCellValue(columns[i]); sheet.setColumnWidth(i, 20 * 256); }
    ...
    ApiResponse<?> upload(HttpServletRequest request, @RequestParam("file") MultipartFile file) throws IOException {
        ...
        try (var input = new BufferedInputStream(file.getInputStream()); var workbook = WorkbookFactory.create(input)) {
            ...
            for (int rowIndex = 1; rowIndex <= sheet.getLastRowNum(); rowIndex++) {
                ...
                if (StreamSupport.empty(username,email,name)) continue; total++;
                try { var user=users.create(tenant,new PoolUserService.Input(username,email,blank(password),blank(phone),name,null,null,blank(department),blank(position),null)); imported.add(Map.of("username",user.username(),"email",user.email(),"name",user.name())); }
    ...
    private static final class StreamSupport{static boolean empty(String... values){return Arrays.stream(values).allMatch(String::isBlank);}}
}
